...
describe('StatsPanel error type summary', () => {
  it('shows error ball types without a tracked point endings section', () => {
    ...
    expect(html).toContain('--fh-share:100%')
    expect(html).toContain('Error ball types')
    expect(html).toContain('aria-label="Neutral: 3 errors, 100%; FH 3, BH 0"')
  ...
  it('keeps serve outcomes separate and excludes double faults from error ball-type percentages', () => {
    ...
    expect(html).toContain('aria-label="Neutral: 1 error, 50%; FH 1, BH 0"')
    expect(html).toContain('aria-label="Slice: 1 error, 50%; FH 1, BH 0"')
  })
})
...
describe('StatsFilters', () => {
  it('uses one row per category, removes All chips, and shows every ball type', () => {
    ...
    expect(html).toContain('class="stats-filter-pie-sector"')
    expect(html).toContain('role="button" tabindex="0"')
    expect(html).toContain('class="stats-filter-orbit-label')
    expect(html).toContain('class="stats-filter-pie-layout labels-below"')
    expect(html).toContain('class="stats-filter-below-labels"')
