...
function Marker({ p: pt, rotation, compact, selected = false }: { p: Point; rotation: CourtRotation; compact?: 'past' | 'analysis' | 'overview'; selected?: boolean }) {
  ...
  const opponentWinner = p.outcome === 'winner'
  const markerKind = opponentWinner ? 'opponent-winner' : forcedError ? 'forced-error' : undefined
  const markerSymbol = opponentWinner ? 'star' : forcedError ? 'cross' : undefined
  ...
  if (compact) {
    ...
    return (
      <g transform={uprightAt(p.x, p.y, false, rotation)} data-marker-kind={markerKind} data-marker-symbol={markerSymbol}>
        <title>{label}</title>
        {selected && (
          <circle
            cx={p.x}
  ...
  return (
      ...
      {/* Colour carries her stroke; a cross marks a forced error. Opponent winners use the
