...
describe('shared match links', () => {
  it('round-trips match metadata and live marks without account identifiers', () => {
    const playerWinner: Point = { ...point, id: 'point-2', outcome: 'player_winner', error_type: '', stroke: 'serve', shot_type: 'ace' }
    const winningServe: Point = { ...point, id: 'point-3', outcome: 'winning_serve', error_type: '', stroke: 'serve', shot_type: 'winning_serve' }
    const shared = decodeSharedMatch(encodeSharedMatch(session, [point, playerWinner, winningServe]))
    expect(shared).toMatchObject({
      ...
      points: [
        ...
        { user_id: null, outcome: 'player_winner', stroke: 'serve', shot_type: 'ace' },
        { user_id: null, outcome: 'winning_serve', stroke: 'serve', shot_type: 'winning_serve' },
      ],
    })
  })
