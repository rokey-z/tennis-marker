import { isErrorType, isOutcome, isPlacementResult, isPlacementStroke, isPointShotType, isSessionMode, type Point, type Session } from './types'
...
export function decodeSharedMatch(encoded: string): SharedMatch | null {
  try {
    ...
    for (const [index, p] of raw.p.entries()) {
      ...
      if (!Array.isArray(p) || p.length !== (v2 ? 9 : 8) || typeof p[0] !== 'number' || typeof p[1] !== 'number' || !isPlacementStroke(p[2]) && p[2] !== '' || !isErrorType(p[3]) && p[3] !== '' || typeof p[4] !== 'boolean' || !isOutcome(p[5]) || p[6] !== null && !isPlacementResult(p[6]) || shotType !== null && !isPointShotType(shotType) || typeof createdAt !== 'string') return null
